...
3. Add a new, separate code cell after the video inference cell to convert the newest `runs/detect/predict*` output to MP4 using ffmpeg. The cell should:
	- verify `ffmpeg` exists (otherwise print an install hint and exit)
	- find the newest `runs/detect/predict*` folder and newest video inside it
...
2. Run the video inference cell to generate an output in `runs/detect/predict*/`.
3. Run the new MP4 conversion cell and verify it produces an `.mp4` file in `runs/detect/predict*/converted_mp4/`, and logs the output path and size.
